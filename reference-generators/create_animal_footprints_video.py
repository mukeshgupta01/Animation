...
WORK = ROOT / "animal-footprints-work"
BACKGROUND = ROOT / "footprint-trail-background.png"
SHEET = ROOT / "footprint-animal-sheet.png"
SILENT = WORK / "animal-footprints-silent.mp4"
OUTPUT = ROOT / "who-left-these-footprints.mp4"
CONTACT = ROOT / "animal-footprints-contact-sheet.png"
W, H, VIDEO_FPS, ART_FPS = 1280, 720, 24, 8
VOICE = "en-AU-NatashaNeural"
RATE = "-8%"

ROUNDS = [
    {"answer": "duck", "choices": ["rabbit", "duck", "horse"],
     "question": "Round one. Rabbit, duck, or horse. Who left these footprints?",
     "answer_line": "It was the duck! Webbed toes make a wide, splashy footprint.",
     "fact": "Webbed toes make a wide, splashy print."},
    {"answer": "rabbit", "choices": ["horse", "bear", "rabbit"],
     "question": "Round two. Horse, bear, or rabbit. Who left these footprints?",
     "answer_line": "It was the rabbit! Its long back feet land ahead of its smaller front feet.",
     "fact": "Long back feet land ahead of the front feet."},
    {"answer": "bear", "choices": ["bear", "lion", "elephant"],
     "question": "Round three. Bear, lion, or elephant. Who left these footprints?",
     "answer_line": "It was the bear! A bear print has a broad pad and five visible toes.",
     "fact": "A broad pad with five visible toes."},
    {"answer": "horse", "choices": ["lion", "elephant", "horse"],
     "question": "Round four. Lion, elephant, or horse. Who left these footprints?",
     "answer_line": "It was the horse! The curved hoof leaves a strong U-shaped mark.",
     "fact": "A curved hoof leaves a U-shaped mark."},
    {"answer": "elephant", "choices": ["bear", "elephant", "horse"],
     "question": "Round five. Bear, elephant, or horse. Who left these footprints?",
     "answer_line": "It was the elephant! Its huge round feet leave large, almost circular prints.",
     "fact": "Huge round feet leave almost circular prints."},
    {"answer": "lion", "choices": ["lion", "bear", "rabbit"],
     "question": "Final round. Lion, bear, or rabbit. Who left these footprints?",
     "answer_line": "It was the lion! Four rounded toes sit above a large paw pad, usually without claw marks.",
     "fact": "Four toes and a large pad, usually no claw marks."},
]
...
def extract_animals():
    ...
    names = [["duck", "rabbit", "bear"], ["horse", "elephant", "lion"]]
    result = {}
    for row in range(2):
        for col in range(3):
...
def draw_round(frame, spec, t):
    ...
    centered(d, (640, 67), "WHO LEFT THESE FOOTPRINTS?" if not revealed else "THE TRACKS BELONG TO...", F30, COLORS["navy"])
    cover(frame, (420, 105, 860, 443), fill=(255, 249, 220, 235), radius=34, width=5)
    if revealed:
        paste_contain(frame, ANIMALS[spec["answer"]], (505, 122, 775, 405))
...
def frame_at(t):
    ...
    if t < ROUNDS[0]["start"]:
        ...
        centered(d, (640, 250), "WHO LEFT THESE", F42, COLORS["navy"], 1)
        centered(d, (640, 325), "FOOTPRINTS?", F54, COLORS["red"], 1)
        centered(d, (640, 410), "6 playful animal track puzzles", F30, COLORS["teal"])
        centered(d, (640, 475), "Look closely and follow the clues!", F24, COLORS["navy"])
    else:
        ...
        if spec:
            ...
            centered(d, (640, 245), "AMAZING TRACKING!", F54, COLORS["red"], 1)
            centered(d, (640, 330), "HOW MANY DID YOU GUESS?", F42, COLORS["navy"])
            for i, name in enumerate(["duck", "rabbit", "bear", "horse", "elephant", "lion"]):
...
async def make_speech():
    ...
    texts = [("intro", "Who left these footprints? Let's follow the tracks and find out!")]
    for i, r in enumerate(ROUNDS, 1):
        texts += [(f"round{i}", r["question"]), (f"answer{i}", r["answer_line"])]
    texts += [("score", "Wonderful tracking! How many of the six animals did you guess correctly?"),
              ("subscribe", "Please like and subscribe for more fun puzzles. See you next time!")]
    for key, words in texts:
        path = WORK / f"animal-footprints-voice-{key}.mp3"
        if not path.exists():
...
def build_timeline(texts):
    ...
    durations = {key: duration(WORK / f"animal-footprints-voice-{key}.mp3") for key, _ in texts}
    ...
    (WORK / "animal-footprints-response-gap-audit.txt").write_text("\n".join(
    ...
    (WORK / "animal-footprints-timing-audit.txt").write_text("\n".join(report), encoding="utf-8")


def make_audio():
    sr = 24000; n = int(DURATION * sr)
    notes = [261.63, 329.63, 392.0, 349.23, 293.66, 349.23]
    bed = WORK / "animal-footprints-bed.wav"; sfx = WORK / "animal-footprints-sfx.wav"
...
def mix_audio():
    inputs = ["-i", str(SILENT), "-i", str(WORK / "animal-footprints-bed.wav"), "-i", str(WORK / "animal-footprints-sfx.wav")]
    filters = ["[1:a]volume=.50[bed]", "[2:a]volume=.90[sfx]"]; labels = ["[bed]", "[sfx]"]
    for idx, (key, start, _) in enumerate(LINES, 3):
        inputs += ["-i", str(WORK / f"animal-footprints-voice-{key}.mp3")]
...
def main():
    global BASE, ANIMALS, STAMPS
    WORK.mkdir(exist_ok=True); BASE = fit_image(BACKGROUND); ANIMALS = extract_animals()
    STAMPS = {name: footprint_stamp(name) for name in ANIMALS}
