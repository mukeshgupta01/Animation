...
WORK = ROOT / "hidden-animals-work"
PUZZLE = ROOT / "hidden-animals-puzzle-corrected.png"
SILENT = WORK / "hidden-animals-silent.mp4"
OUTPUT = ROOT / "find-5-hidden-animals.mp4"
...
LINES = [
    ("hello", 0.45, "Hello, kids! Are you ready for a fun challenge?"),
    ("five", 5.75, "There are five sneaky animals hiding somewhere in this picture."),
    ("timer", 11.90, "Can you find all five before the timer runs out?"),
    ("careful", 16.85, "Look very carefully... because some of them are really good at hiding!"),
    ("ready", 23.80, "Ready?"),
    ("starts", 25.70, "Your time starts... now!"),
    ("clue1", 34.00, "Clue one... look in the big green tree."),
    ("clue2", 40.50, "Clue two... look inside the pink tree trunk."),
    ("clue3", 47.00, "Clue three... search the flowers at the lower left."),
    ("clue4", 53.50, "Clue four... look beside the stream."),
    ("clue5", 59.80, "Clue five... look near the fence on the right."),
    ("timesup", 69.30, "Time's up! How many animals did you find?"),
    ("monkey", 74.50, "The monkey hid in the left tree. Clue one said to look in the trees."),
    ("owl", 81.00, "The owl hid in the pink tree trunk. Clue two mentioned the pink tree."),
    ("rabbit", 87.90, "The rabbit hid among the lower-left flowers. That was clue three."),
    ("frog", 94.30, "The frog hid beside the stream. That was clue four."),
    ("turtle", 100.40, "The turtle hid near the fence on the right. That was clue five. Great searching!"),
    ("subscribe", 109.20, "Did you enjoy the challenge? Please like and subscribe for more fun videos like this. See you next time!"),
]
...
def clue_card(frame, heading, body, top=False):
    ...
    x1, x2 = (350, 1135) if top else (135, 1145)
...
ANIMALS = [
    ("1  MONKEY", (0.145, 0.140), (233, 63, 72), "Big green tree on the left"),
    ("2  OWL", (0.756, 0.460), (255, 188, 42), "Pink-blossom tree trunk"),
    ("3  RABBIT", (0.145, 0.700), (66, 172, 73), "Flowers at the lower left"),
    ("4  FROG", (0.430, 0.916), (25, 155, 220), "Beside the stream"),
    ("5  TURTLE", (0.707, 0.916), (155, 88, 210), "Near the right fence"),
]
...
def render_video():
    ...
    for n in range(total):
        ...
        if 0.35 <= t < 3.15:
            ...
            rounded_label(frame, "FIND 5 HIDDEN ANIMALS!", 276, "big", number_five=True)
        elif 11.75 <= t < 16.65:
            ...
            clue_card(frame, "LOOK CAREFULLY", "Some are really good at hiding!")
        elif 23.65 <= t < 25.35:
        ...
        if challenge_start <= t < 69.0:
            ...
            if 34.0 <= t < 39.2:
                clue_card(frame, "CLUE 1", "Look in the big green tree.", top=True)
            elif 40.5 <= t < 45.7:
                clue_card(frame, "CLUE 2", "Look inside the pink tree trunk.", top=True)
            elif 47.0 <= t < 52.2:
                clue_card(frame, "CLUE 3", "Search the flowers at the lower left.", top=True)
            elif 53.5 <= t < 58.7:
                clue_card(frame, "CLUE 4", "Look beside the stream.", top=True)
            elif 59.8 <= t < 64.0:
                clue_card(frame, "CLUE 5", "Look near the fence on the right.", top=True)
            elif remaining <= 5:
        ...
        if 69.0 <= t < 74.3:
            ...
            d.text((W//2, 413), "How many animals did you find?", anchor="mm", font=font(29, True), fill=(17,50,88,255))

        reveal_times = [(74.3,80.9), (80.9,87.8), (87.8,94.2), (94.2,100.3), (100.3,109.0)]
        ...
        if 109.0 <= t < DURATION:
            ...
            d.text((W//2, 375), "For more fun challenges", anchor="mm", font=F_MED,
                   fill=(16,52,90,255))
