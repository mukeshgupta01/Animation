...
- CURRENT REQUEST (2026-09-22): produce two new Parenting Rewind videos as one finite interval. Preserve existing Parenting upload settings and all other projects. Read C:/DocSphere/CURRENT_RULES.md for current Birthday preparation decisions; the older 13 September pause note is superseded.
- USER RECONFIRMATION (2026-09-13): continue making more Parenting Rewind videos under the same varied, evidence-based production and verified-transfer workflow. Latest completed episode numbers and continuation details belong in `PROJECT-HANDOFF.md`.
...
- LATEST PUBLIC UPLOAD CADENCE OVERRIDE (updated 2026-08-31): supersede the two-hour/four-hour cadence above and upload the oldest remaining authorized Parenting Rewind episode as `public` once every hour. The task `Parenting Rewind - Public Upload Cadence` and uploader gate must both use one hour. Preserve immutable channel `UCGb-IUQX2KQa_KA24MwE_aQ`, `made_for_kids=false`, `containsSyntheticMedia=true`, oldest-first order, and all fail-closed metadata, FFprobe, hash and duplicate checks. The superseded private and five-hour task definitions were removed; do not recreate them.
- BACKGROUND TASK POLICY (2026-08-31): the hourly upload task and every nested helper must run hidden and non-interactively. Preserve logs and Outlook reporting, but never open or flash a Command Prompt or PowerShell window during scheduled work.
- ORDINARY QUEUE ISOLATION (updated 2026-09-08): the hourly public cadence may select only normal files whose names begin `parenting-rewind-`. Separately published special collections copied into the shared OneDrive folder, including the five numbered Father's Day Shorts, use their own guarded journal and must not enter or block the ordinary oldest-episode queue.
- AI DISCLOSURE (2026-08-27): set YouTube's altered/synthetic-content disclosure to Yes (`status.containsSyntheticMedia=true`) for every Parenting Rewind upload. Apply it to all existing videos on the immutable Parenting Rewind channel and include it in every future upload request. Do not apply this instruction to Birthday Songs or Tiny Tales.
...
- FATHER'S DAY SCHEDULED VIDEO (updated 2026-09-02): the user approved the corrected native-9:16 episode 86 exact master and requested a Friday-night Parenting Rewind upload. It is uploaded privately as YouTube video `ezoeeeZd4bs` and API-verified for automatic public release at `2026-09-04T10:00:00Z` (20:00 Australia/Sydney), on immutable channel `UCGb-IUQX2KQa_KA24MwE_aQ`, with `made_for_kids=false` and `containsSyntheticMedia=true`. Do not upload or queue another copy. The dedicated idempotent verifier is `automation/schedule_fathers_day_upload.py`; its ignored journal prevents duplicates. YouTube rejected the custom thumbnail with channel-permission HTTP 403, so the scheduled release currently uses YouTube's generated thumbnail. Preserve the native portrait master and tribute-specific chapter labels.
- FATHER'S DAY 20-SECOND COLLECTION (updated 2026-09-03): the user approved five emotional concepts and explicitly authorized their upload to Parenting Rewind. All five exact 20-second portrait masters are public and remotely verified: His Old Wallet `6zRFGniV4z0`, One More Story `SHOydVIEWK8`, The Empty Chair `B-dauMumHLs`, The Missed Call `d3rX3gdNueU`, and Shoes by the Door `TALHXV--VQY`. Each is `made_for_kids=false` with `containsSyntheticMedia=true`. Do not upload or queue duplicates. YouTube rejected every custom-thumbnail request with the channel's existing HTTP 403 permission restriction; preserve the locally generated thumbnails but do not treat this as an upload failure. Masters and metadata are under `output/fathers-day-20-second-shorts` and `metadata/fathers-day-20-second-shorts`; the idempotent upload journal is ignored under `automation/runtime`.
