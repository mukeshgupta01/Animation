...
## Ten-minute backlog uploads and new-set start (2026-08-27)

- The user explicitly requested that all eligible videos upload with a ten-minute gap. Live task `Tiny Tales - Daily Private Upload` uses one long-running repeating `PT10M` trigger, anchored to a future ten-minute boundary rather than a past midnight boundary. Each run still uploads at most one item through the existing uploader and global mutex. The migration and installer use a 3650-day repetition duration so the cadence does not expire after one day.
- Because every normal cycle checks `runtime/upload-retry-state.json` first and retries the exact duplicate-safe failed source before considering a newer item, the separate `Tiny Tales - Hourly Upload Retry` task is now Disabled rather than deleted. This prevents competing schedules. `automation/Install-TinyTalesTasks.ps1` records the same design for new installs, and `automation/Set-TinyTalesTenMinuteUploads.ps1` safely migrates existing tasks only after verifying their exact actions.
...
- Live verification found the laptop on battery and the historical task settings blocking all battery starts, which caused the first trigger boundaries to advance without launching. The migration and fresh installer now allow this upload task to start and finish on battery; network availability, `IgnoreNew`, the global mutex and the 45-minute execution limit remain active.
...
- `metadata/ant-grasshopper-pika-pilot-02.json` locks a deliberately simpler test: only Ant takes one slow planted weighted step while every other character holds with breathing/blinking only and the camera remains locked. This isolates whether Pika can produce locomotion without prompt overload.
- `metadata/ant-grasshopper-shared-harvest-01-plan.json` now points to both pilots and blocks remaining story-art generation until pilot 02 passes provider motion and character consistency review. `metadata/new-set-plan-2026-08-27.json` records the Ant production as active with original-reference safeguards.
