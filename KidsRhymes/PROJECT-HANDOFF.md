...
- The user explicitly requested that all eligible videos upload with a ten-minute gap. Live task `Tiny Tales - Daily Private Upload` was migrated to one repeating `PT10M` trigger with duration `P1D`; at verification it was Ready and its next run was 13:30 Australia/Sydney. Each run still uploads at most one item through the existing uploader and global mutex.
