...
- Felix production has started under `metadata/felix-firefly-night-light-parade-01-plan.json`. All eight premium 1672x941 scenes are accepted and hash-locked in `metadata/felix-firefly-night-light-parade-01-asset-review.json`. Two raw generations were rejected and never copied into the project: the opening with an extra lantern and the finale with eight tiles/four lanterns. Accepted continuity ends with exactly three lanterns and two exact three-leaf colour patterns. Nothing is rendered, queued or uploaded yet; producer, soundtrack, pacing audit, all final quality evidence and thumbnail remain pending.
