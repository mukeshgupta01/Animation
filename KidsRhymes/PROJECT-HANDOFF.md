...
- The tracked review is `metadata/cardboard-box-invention-club-01-asset-review.json`. The producer, narration, music, MP4, quality audits, thumbnail and manifest entry do not exist yet. The item is deliberately not in `automation/generation-manifest.json`, so the exhausted automatic generation task continues to no-op and cannot queue an unfinished video.
