...
## Corrected uploads, performance finding and next visual direction (2026-08-29)

- The corrected slow-narration masters were released through the existing safeguarded `PT15M` task and uploaded publicly/made-for-kids on verified Tiny Tales: Fruit Picnic `aPtUBaZrPcI` at 10:30 Sydney, Dad's Welcome `2CKnDBpjFlQ` at 10:45, and Eddie Excavator `ggJqahpLAtg` at 11:00. Exact reviewed hashes match the ledger. A final dry run reported zero failures, zero remaining uploads and no next video.
- A read-only all-channel audit found 187 live uploads, but project isolation narrowed performance analysis to the 59 IDs in this Tiny Tales upload ledger. The current leaders by public views are Animal Opposites Playground `UleavxVG2T4` / 574, Animal Alphabet A-Z `-ayS0UTmcfA` / 472, Farm Animal Counting Parade `1bUWqBg9V9g` / 461, Tiny Seed `uXSMNcg-u10` / 435 and Maya/Joey `WLzesx1OxNU` / 378. Exact watch-time minutes are unavailable because the current token has upload/read-only scopes, not YouTube Analytics scope.
- Four newly uploaded low-view videos were selected and received reviewed local title/thumbnail replacement plans: Rory, Five Ducks, Lina and Ant/Grasshopper. The live `videos.update` call failed with HTTP 403 `insufficientPermissions` before any mutation because editing a title needs broader YouTube channel-management OAuth scope. No title or thumbnail was partially changed. Preserve `metadata/low-view-title-thumbnail-refresh-2026-08-29.json`, its four reviewed JPEGs and `automation/refresh_low_view_metadata.py`; rerun the dry run only after a deliberate one-time OAuth scope expansion and exact channel verification.
- Built-in image generation created four stronger low-view thumbnail artworks and three additional accepted Lumi theatre scenes, then reached its team usage limit. The tool reported reset at 2026-09-04 05:29:49 Australia/Sydney. Do not substitute paid/manual providers; the user still requires zero-payment, no-manual production.
- Lumi now has five local reviewed scene compositions but is paused because the user prioritized a different-animal extension of the strongest opposites signal. `produce_lumi_shadow_theatre.py` is an unfinished local producer and must not be rendered, queued or uploaded without resuming and reviewing that direction.
- The new direction is `quinn-quokka-opposite-parcel-quest-01`: a connected parcel-delivery story using empty/full, up/down, near/far, fast/slow, open/closed and light/heavy. It deliberately changes cast, story, settings, interaction and original music rather than cloning Animal Opposites Playground. The exact composition plan is `metadata/quinn-quokka-opposite-parcel-quest-01-plan.json`.
- The first seven-frame Quinn local composite preview was rejected. Although the story structure is distinct, the parcel controls looked flat beside premium environments and Quinn repeated one pose too often. Do not render, queue or upload this preview. Revision requires scene-specific Quinn/Olive poses, tactile dimensional parcel props, integrated lighting and physical cause/effect. The user explicitly asked for better-looking visuals and said a similar concept must not be the same video with a different animal.

## Scheduled Task cleanup (2026-08-31)

- A live audit inspected every installed task whose name or action referenced Animation, KidsRhymes, Parenting, Birthday, YouTube or Git sync. Five project-related tasks were found.
- `Tiny Tales - Hourly Upload Retry` was the only unused task: it had been Disabled since 2026-08-27, its retry state was clear, and the active repeating `PT15M` uploader already services the same exact-source duplicate-safe retry state before choosing newer work. The exact disabled task was unregistered at the user's request.
- `automation/Install-TinyTalesTasks.ps1` and `automation/Set-TinyTalesFifteenMinuteUploads.ps1` no longer create, require or disable the retired retry task. Do not reinstall it.
- Preserve the four active tasks: `Tiny Tales - Continuous Generation`, `Tiny Tales - Daily Private Upload`, `Tiny Tales - Daily Upload Summary Email`, and workspace task `Animation Git Sync Every Three Hours`. At audit time all four had successful last results; generation was an exhausted-manifest no-op, the upload queue was empty, the summary task sent the required previous-day count, and Git sync was current.

## Fifteen-video premium musical set authorized (2026-08-31)

- The user explicitly authorized creation of all fifteen concepts from Nellie Narwhal through Benny Bilby and direct release to the verified Tiny Tales channel. The authoritative set plan, production order, voice rotation, BPMs, art systems and shared quality gates are in `metadata/premium-musical-video-set-2026-08-31.json`.
- Direct release means public and made for kids through the existing channel-locked `PT15M` task. It does not waive any visual, narration-pacing, synchronization, transition, decode, audio, thumbnail or read-back check. An item enters `generation-manifest.json` only after its producer and required premium assets are ready.
- Production begins with Felix Firefly, Basil Beaver and Gus Gecko because their luminous-pattern, physical-water-engineering and gravity-aware museum systems are strongly differentiated from the covered catalog. The remaining twelve retain separate settings, mechanics, motion languages, emotional arcs and rotating lead voices.
- Felix is complete and live at `https://youtu.be/qYQHMgq61Fs`. The 88-second master passed full decode, 1080p H.264/48 kHz stereo, zero-gap/transition, narration containment and pacing gates (127.88 weighted WPM, 133.22 maximum line WPM, 0.552-second minimum phrase gap), plus manual review of the general sheet, every cut, final-only climax and revised custom thumbnail. Exact master SHA-256 is `340a13fdb13501f8c0bb1306a9509870ccf131f94174377719e2ee0fb8a05030`; reviewed thumbnail SHA-256 is `1f45c0d2a3008d81749ed52f855dba044ae5753f2f3bd13b3f188ce3bff7c489`. The safeguarded 18:15 scheduled uploader published it public/made-for-kids to locked channel `UCEn9N-ITQHshjgt6fy7fxnw`; exact API read-back confirmed the title and status, and the custom-thumbnail upload is ledgered. Do not regenerate Felix or retry its upload.
- Basil is complete and live at `https://youtu.be/TStyNqaucQQ`. The 102.18-second 88 BPM master uses nine distinct tactile river-workshop scenes and an original marimba/wood-percussion story-song. A first render was rejected solely because ten seven-word lines exceeded the per-line pace ceiling; corrected six-word phrases reduced pacing to 129.81 weighted WPM and 133.36 maximum line WPM with 0.700-second minimum gaps. Full decode, 1080p H.264/48 kHz stereo, zero-gap/transition, lyric/effect containment and all technical checks passed. Manual review confirmed material truth (leaf floats; stones sink but leave gaps; five sticks frame; reeds slow; clay seals), stopped wheel through scene 7, first wheel payoff in scene 8, open fish passage, scene-9-only band, final-only end card, waveform and unobstructed thumbnail. Exact master SHA-256 is `af4ba483369a7aadacba0648099f40f8766b19f3197d9553275d7646a85b63a5`; thumbnail SHA-256 is `dcbc0f1a94200a670915a0c3dbf013117e0358e25cefb7784450cf63d3da29d0`. Direct safeguarded upload and exact API read-back confirmed public/made-for-kids status on the locked channel. Do not regenerate or retry Basil.
- Gus is complete and live at `https://youtu.be/mKIhJftPCoo`. The exact 94-second 96 BPM master uses nine jewel-toned museum scenes and an original pizzicato/glass chamber caper. One scene-8 image was rejected because it showed beetle above shell and left treasures on the trolley; the corrected four-bay audit simultaneously preserves above, below, beside and between. A first render was rejected solely for 0.14-second phrase gaps; six shortened lines and 2.7-second windows corrected pacing to 127.52 weighted WPM, 134.21 maximum line WPM and 0.437-second minimum gaps. Full decode, 1080p H.264/48 kHz stereo, zero-gap/transition, containment, 27 real effects and all automated checks passed. Manual review confirmed stable horizons, all six spatial relationships, scene-9-only visitors/music, Gus's landed final pose, final-only end card, waveform and unobstructed thumbnail. Exact master SHA-256 is `1ef800d545d0ddefa3ee1730d2a64e745f6e8670cffb9d0d9d5382ccd82b68a3`; thumbnail SHA-256 is `daef61ab0efd82d6226f790233fbf4b651993d4b577fafa256da889a21191319`. Direct safeguarded upload and exact API read-back confirmed public/made-for-kids status on the locked channel. Do not regenerate or retry Gus.
- Nellie is complete and live at `https://youtu.be/oQlxxCIzIpM`. The 104.47-second 86 BPM master uses nine premium cinematic Arctic compositions and an original glass-harp, strings and soft-percussion colour-rescue ballad with Maisie (UK), Natasha (AU) and Ana (US). All nine source scenes were manually reviewed and hash-locked. The full master passed decode, 1080p H.264/48 kHz stereo, zero-gap/transition, lyric/effect containment and narration pacing gates at 116.74 weighted WPM, 143.09 maximum line WPM and 0.547-second minimum gaps. Manual review confirmed stable floe geography, exact green-blue-violet route order, Kiko safely on ice through scene 7, first swim in scene 8, scene-9-only reunion/full aurora, final-only end card, contained waveform/spectrum and an unobstructed custom thumbnail. Exact master SHA-256 is `9e7ea66829d737741b39074ab13b85dadbed0e85b833df2bf2f39551ca13bbfe`; thumbnail SHA-256 is `86a1ca78161f90d15736e192e93a5b7a3521f8268519f5620abae25fb5182891`. Direct safeguarded upload and independent API read-back confirmed processing succeeded, exact title/description/seven tags, PT1M45S duration, public visibility, made-for-kids and self-declared-made-for-kids status on locked Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`. The upload queue is empty. Continue next with Tilly Turtle; do not regenerate or retry Nellie.
- Nellie is complete and live at `https://youtu.be/oQlxxCIzIpM`. The 104.47-second 86 BPM master uses nine premium cinematic Arctic compositions and an original glass-harp, strings and soft-percussion colour-rescue ballad with Maisie (UK), Natasha (AU) and Ana (US). All nine source scenes were manually reviewed and hash-locked. The full master passed decode, 1080p H.264/48 kHz stereo, zero-gap/transition, lyric/effect containment and narration pacing gates at 116.74 weighted WPM, 143.09 maximum line WPM and 0.547-second minimum gaps. Manual review confirmed stable floe geography, exact green-blue-violet route order, Kiko safely on ice through scene 7, first swim in scene 8, scene-9-only reunion/full aurora, final-only end card, contained waveform/spectrum and an unobstructed custom thumbnail. Exact master SHA-256 is `9e7ea66829d737741b39074ab13b85dadbed0e85b833df2bf2f39551ca13bbfe`; thumbnail SHA-256 is `86a1ca78161F90D15736E192E93A5B7A3521F8268519F5620ABAE25FB5182891`. Direct safeguarded upload and independent API read-back confirmed processing succeeded, exact title/description/seven tags, PT1M45S duration, public visibility, made-for-kids and self-declared-made-for-kids status on locked Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`. Do not regenerate or retry Nellie.
- Tilly is complete and live at `https://youtu.be/4hMcSA6koBA`. The 92.16-second 98 BPM master uses nine premium stop-motion-inspired coastal bakery compositions, an original pizzicato/marimba/tactile-percussion song, and four rotated voices: Ana (US), Maisie (UK), Ryan (UK) and Natasha (AU). Thirteen image-generation calls produced nine accepted hash-locked scenes; four unused or defective variants were rejected, including a missing Pip, incorrect square half-loaves and scarf drift. The full master passed decode, 1080p H.264/48 kHz stereo, zero-gap/four-bar transition, lyric/effect containment and narration pacing gates at 128.00 weighted WPM, 142.16 maximum line WPM and 0.523-second minimum gaps. Manual review confirmed exact cart/customer route, warm-cool and soft-crunchy food truth, one whole loaf becoming exactly two semicircular halves, scene-9-only full bakery band, final-only end card and contained waveform/spectrum. The first thumbnail was rejected for clipped text; corrected `WARM OR COOL?` is fully legible and unobstructed. Exact master SHA-256 is `4c8d6e863f0fc0d9e9ae9d038421f095ee4f6657bf7d016e8f6cf657966dedd5`; corrected thumbnail SHA-256 is `1960baf108c53cfb3c5d03e722232b753379a17b26db0d934dd0d15eb93e3d2b`. The normal 23:15 Sydney safeguarded cycle uploaded it; independent API read-back confirmed processing succeeded, exact title/description/seven tags, PT1M33S duration, public visibility, made-for-kids and self-declared-made-for-kids status on the locked channel, and the custom-thumbnail ledger confirms the corrected image. The queue is empty. Continue next with Pogo Penguin; do not regenerate or retry Tilly.
- Tilly is complete and live at `https://youtu.be/4hMcSA6koBA`. The 92.16-second 98 BPM master uses nine premium stop-motion-inspired coastal bakery compositions, an original pizzicato/marimba/tactile-percussion song, and four rotated voices: Ana (US), Maisie (UK), Ryan (UK) and Natasha (AU). Thirteen image-generation calls produced nine accepted hash-locked scenes; four unused or defective variants were rejected, including a missing Pip, incorrect square half-loaves and scarf drift. The full master passed decode, 1080p H.264/48 kHz stereo, zero-gap/four-bar transition, lyric/effect containment and narration pacing gates at 128.00 weighted WPM, 142.16 maximum line WPM and 0.523-second minimum gaps. Manual review confirmed exact cart/customer route, warm-cool and soft-crunchy food truth, one whole loaf becoming exactly two semicircular halves, scene-9-only full bakery band, final-only end card and contained waveform/spectrum. The first thumbnail was rejected for clipped text; corrected `WARM OR COOL?` is fully legible and unobstructed. Exact master SHA-256 is `4c8d6e863f0fc0d9e9ae9d038421f095ee4f6657bf7d016e8f6cf657966dedd5`; corrected thumbnail SHA-256 is `1960baf108c53cfb3c5d03e722232b753379a17b26db0d934dd0d15eb93e3d2b`. The normal 23:15 Sydney safeguarded cycle uploaded it; independent API read-back confirmed processing succeeded, exact title/description/seven tags, PT1M33S duration, public visibility, made-for-kids and self-declared-made-for-kids status on the locked channel, and the custom-thumbnail ledger confirms the corrected image. Do not regenerate or retry Tilly.
- Pogo is complete and live at `https://youtu.be/EJVfLAdz_2s`. The 92.27-second 104 BPM master uses nine premium tactile Antarctic engineering compositions, an original pizzicato/marimba/snow-percussion theatre march and Ryan (UK), Ana (US) and Natasha (AU) voices. Twelve image-generation calls produced nine accepted hash-locked scenes; three defective or superseded variants were rejected, including both rails on one side, Mina and the sled incorrectly joining the first crossing, and a finale before Pogo's golden cheek patch was restored. The full master passed decode, 1080p H.264/48 kHz stereo, zero-gap/beat-grid transitions, lyric/effect containment and pacing gates at 133.26 weighted WPM, 142.16 maximum line WPM and 0.635-second minimum gaps. Manual review confirmed the visibly shallow powder trench, narrow-before-wide test, three supports, one rail on each opposite side, two-sack load test, teal X brace, Pogo-only scene-8 crossing, scene-9-only band, final-only end card, contained waveform/spectrum and unobstructed `BUILD IT STEADY!` thumbnail. Exact master SHA-256 is `fffdfa340df9deeaede724e9019ab8a6ee93c64d41e0ee114abc34ea91e229c9`; thumbnail SHA-256 is `587777ec361a0e013b49193072c7dba8bbb108a366cbfeb63ad23c8ac50ffb96`. Direct safeguarded upload and independent API read-back confirmed processing succeeded, exact title/description/seven tags, PT1M33S duration, public visibility, both made-for-kids flags and locked Tiny Tales channel. The queue is empty. Continue next with Zara Zebra; do not regenerate or retry Pogo.
- Zara is complete and live at `https://youtu.be/qYuipr3PBt4`. The 95.8-second 100 BPM master uses nine premium cinematic savannah-town scenes, an original marimba/brass-bell/hand-percussion/flute street song and Natasha (AU), Ana (US) and Maisie (UK) voices. Fourteen image-generation calls produced nine accepted hash-locked scenes; five rejected or failed generations included incorrect five-, seven- and eight-panel crossings and one harmless finale moderation failure. The full master passed decode, 1080p H.264/48 kHz stereo, zero-gap/beat-grid transitions, lyric/effect containment and pacing gates at 122.73 weighted WPM, 142.15 maximum line WPM and 0.762-second minimum gaps. Manual review confirmed raised-curb and blocked-cart safety, six stripes before public crossing, one resonator per stripe from tuning onward, curb-only call-and-response, one-family first crossing, one-direction procession, scene-9-only full concert, final-only end card, waveform/spectrum and unobstructed `STEP TO THE BEAT!` thumbnail. Exact master SHA-256 is `4eda4fc00f00f2b0615533f835b11f61c8d4262c2a21421417d95a516ba8a7f1`; thumbnail SHA-256 is `370264aa7a15bda8abf46ccf045b6f8e1f027d2926409d0395fe70643ca24115`. Direct safeguarded upload and independent API read-back confirmed HD processing, exact title/description/seven-tag set, PT1M36S duration, public visibility, both made-for-kids flags and locked Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`. The queue is empty. Continue next with Ollie Otter; do not regenerate or retry Zara.

## Seven-video private review hold (2026-09-01)

- The user reported that the videos added during the last two days looked weird and asked that the affected videos be made private during review. Exact channel/title/status preflight verified Tiny Tales `UCEn9N-ITQHshjgt6fy7fxnw` and the seven intended IDs: Felix `qYQHMgq61Fs`, Basil `TStyNqaucQQ`, Gus `mKIhJftPCoo`, Nellie `oQlxxCIzIpM`, Tilly `4hMcSA6koBA`, Pogo `EJVfLAdz_2s`, and Zara `qYuipr3PBt4`.
- Pogo and Zara were already private at preflight. Felix, Basil, Gus, Nellie and Tilly were changed from public to private, and a final API read-back confirmed all seven private and still made for kids on the locked channel. Do not republish any of these seven without explicit user approval after review.
- The credential required a deliberate one-time OAuth expansion to `youtube.force-ssl` because upload/read-only scopes cannot update privacy. `automation/uploader.py` now declares that scope in addition to upload/read-only, and the refreshed credential was independently channel-verified. Never copy the credential or use it outside Tiny Tales.
- Review finding: the seven masters are technically valid but share a weak motion system. Each scene is a single generated tableau held for roughly 10-11 seconds, with only a 3.5% eased whole-frame crop and small particle accents. Narrated actions such as building, carrying, crossing and swimming are not animated as primary actions, so the result reads as a slow moving slideshow. Automated continuity checks proved timing coverage and file integrity, but did not prove character performance or meaningful motion.
- Use `automation/set_recent_video_privacy.py` for exact-target dry-run/read-back if this hold must be audited. It is fail-closed on immutable channel, exact ID/title, made-for-kids state and explicit confirmation.

## Seven corrected semantic-motion replacements (2026-09-01)

- The user explicitly asked to fix all seven and republish them. Each producer now uses `automation/production/semantic_motion.py` to present an independently moving, identity-locked foreground cast over a defocused contextual storybook backdrop. Every scene has visible start/action/end state changes plus story-specific moving objects; whole-frame crop movement and ambient particles no longer qualify as the primary animation.
- All seven corrected masters passed their existing 1080p H.264, 48 kHz stereo AAC, timeline, pacing, thumbnail and full-FFmpeg-decode gates. The new semantic evidence consists of `semantic-motion-contact-sheet.png` and `semantic-motion-audit.json` in each production-work folder. Every exact corrected hash was manually reviewed before its metadata flags were released.
- The corrected public/made-for-kids replacements are: Felix `0CacEgX5rAg` (`5194d90b52c6d8ec3cd7f480eab4b7cd911530d33ad024f5058920b5d9b65363`), Basil `lwD7Zhn47W4` (`78a5fe5c7690e32eba4617c675377842012303f16e684c76b9813fb5b10aa669`), Gus `tdgKPtS8PJI` (`ec6b9e2b20f41c942f73316677c618a0c5cbfec9b0cc662d3a7a47b5e39be70f`), Nellie `RBuSJYnlFLk` (`8a103b91208628f332702b4c63fe5290b9489eaed6190581f1a1df73b9bd400a`), Tilly `g0C7_xVAqlo` (`ac97c7ddd45fcb9119f585b159b57fde1c62320544cde31d33c903456d5ba372`), Pogo `5q5TjZRHQkM` (`fed792fbb22acebee2bc9470d85dea7ce368749445ec04ffc7cb4d1ca4a05ee5`), and Zara `tn7EAIhLiYQ` (`3aa49efccd98671511b291495e0aca5f9777de4fda53fc04c8256425e9ef81c1`). Reviewed custom thumbnails were applied to all seven.
- A final independent API read-back, recorded in ignored `automation/runtime/recent-replacement-verification.json`, confirmed all replacements public and made for kids on Tiny Tales, with matching titles and durations. It also confirmed all seven defective originals remain private and made for kids. Deleting the originals was not approved by the security gate, so they were preserved privately for audit/recovery rather than removed.
- `automation/config.json`, `automation/uploader.py` and `automation/generation_runner.py` now enforce a permanent semantic-motion cutoff. New videos must include four explicit reviewed flags and a complete per-scene audit; the uploader rejects camera-only motion, unnamed foreground movement, continuity failures, or unreviewed scenes. `automation/approve_semantic_replacements.py` keeps rendering and approval separate so a build cannot self-approve its own evidence.
- The upload task was disabled during replacement to prevent a race, restored to Ready afterward, and the pending queue is empty. Preserve the corrected archives, the original defective archives/uploads, both upload ledger histories and all exact IDs.

## Faulty-upload deletion and Felix-only recovery (2026-09-02)

- The user explicitly rejected the original seven uploads and their seven semantic-motion replacements, then explicitly authorized deletion of all 14. Exact channel/title/private/made-for-kids preflight was performed for each ID, exactly one delete request was issued per target, and a final read-only query confirmed all 14 absent from Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`. The ignored audit report is `automation/runtime/delete-fourteen-faulty-uploads.json`; the guarded helper is `automation/delete_fourteen_faulty_uploads.py`.
- Deleted pairs: Felix `qYQHMgq61Fs`, `0CacEgX5rAg`; Basil `TStyNqaucQQ`, `lwD7Zhn47W4`; Gus `mKIhJftPCoo`, `tdgKPtS8PJI`; Nellie `oQlxxCIzIpM`, `RBuSJYnlFLk`; Tilly `4hMcSA6koBA`, `g0C7_xVAqlo`; Pogo `EJVfLAdz_2s`, `5q5TjZRHQkM`; Zara `qYuipr3PBt4`, `tn7EAIhLiYQ`. Local archives remain intentionally preserved for audit.
- Diagnosis was objective as well as visual. The rejected Felix opening measured about `-43.0 dB` RMS above 12 kHz, roughly 36-38 dB noisier than accepted Ant/Grasshopper (`-79.2 dB`) and Five Little Ducks (`-80.9 dB`). The source used unfiltered random wing/wind/shaker noise; the replacement reused that audio. The replacement also used a blurred background plus pasted cutouts, which was visibly below the integrated full-frame quality of accepted work.
- Recovery is now one video at a time. The first local candidate is new ID `felix-firefly-three-lantern-trail-02`, produced by `automation/production/produce_felix_three_lantern_story.py` from `metadata/felix-firefly-three-lantern-trail-02-plan.json`. It is a conventional eight-scene cause-and-effect story: Felix finds three dark lanterns, the friends rebuild two patterns, overcome a gust and Felix's fatigue, light the final lantern, and guide neighbours home.
- Local master SHA-256 is `d2c70155de568232ef2e16ae5b76d2847875e51f59f7ae4289c90879877ea5b4`, duration 100.0 seconds. Full decode, 1080p H.264/48 kHz stereo AAC, zero gaps, beat-grid starts, containment, eight unique scenes, title/problem opening and child pacing passed. Narration is 117.08 weighted WPM, 139.75 maximum WPM, with a 0.448-second minimum gap; audio is -16.0 LUFS / -1.4 dB true peak. The opening above-12-kHz RMS is `-105.30 dB`, passing the new `-65 dB` hiss ceiling.
- The new opening art was created with the built-in image generator using the existing Felix identity and Ant/Grasshopper finish as references, and is saved as `automation/production-assets/felix-story-rebuild-opening-v3.png`. It shows Felix arriving at Rabbit and Hedgehog beside exactly three dark lanterns on the lost moonlit path. General and transition sheets were inspected locally and show integrated full-frame story compositions with no blurred duplicate background or pasted cutouts.
- The candidate remains deliberately unreleased: metadata privacy is private, `upload_authorized` and `upload_queue_released` are false, and every manual-review flag remains false. Do not upload or publish it until the user reviews the exact MP4/thumbnail and explicitly accepts this visual/story/audio standard. Do not begin Basil through Zara until Felix is accepted.

## Felix release and Father's Day start (2026-09-02)

- The user accepted the one-at-a-time Felix rebuild for upload while noting it was only acceptable, not a new quality ceiling. Exact master `d2c70155de568232ef2e16ae5b76d2847875e51f59f7ae4289c90879877ea5b4` uploaded public/made-for-kids as `h2RfEK01Rr0` (`https://youtu.be/h2RfEK01Rr0`) with thumbnail hash `94c75de299e89b89be094a32250e4c07fa46a23b9f3c7e4ce9c9c27fa9a33fe7`. Independent API read-back passed exact title, description, tags, category, channel, public status, both made-for-kids flags, HD processing and PT1M41S duration. The queue is empty.
- The user then requested a fun, lively Father's Day song featuring animal children and their dads. Work has started as `dance-with-dad-animal-parade-01`: an original 120 BPM call-and-response park parade with elephant stomp, penguin slide, fox spin, kangaroo bounce and lion drum verses, accumulating into one ensemble chorus and affectionate finale. The authoritative plan is `metadata/dance-with-dad-animal-parade-01-plan.json`.
- Built-in image generation created and local review accepted two matched opening states: `automation/production-assets/dance-with-dad-opening-v1.png` (`b119e823146d36b447651cced357ff569320e50636c0e25c83966c474d5193ce`) and `automation/production-assets/dance-with-dad-opening-action-v1.png` (`7a3e1311bf41718a0d665ada7de982d43a65689ff88c0d75a4d8d405089e8c5b`). Each has exactly five child-dad species pairs, ten total animals, consistent clothing/location/camera/lighting, and no cutout/blur treatment. Review details are in `metadata/dance-with-dad-animal-parade-01-asset-review.json`.
- The user rejected the first V2 vocal mix because its pitch-shifted/vibrato delivery sounded like uninteresting narration. Superseded master SHA-256 `f9ed7f432646aa31d760bf79ed8bed644db2e54944ffb9c7d75d5fee5b4f672d` must not be used.
- The user then explicitly requested a song rather than narration and required the final sung lyric `Happy Father's Day, Dad!`; natural spoken V3 hash `a02303dafb03b147df552f8d44d97cb1fd41ef1c34f775d3de0b1fd9ea15d0e3` is superseded.
- Current V4 at `automation/production-output/dance-with-dad-animal-parade-01.mp4` is exactly 100 seconds, 120 BPM, H.264 1920x1080 and stereo AAC; exact SHA-256 is `42642f48833326c403a703ec1af66612fb8da5f098d414b836b4837746b272af`. It uses four formant-preserved notes per lyric line, explicit rising/falling melodies, chorus harmonies and animal-specific hooks. The exact final phrase is sung from 90-94 seconds, followed only by the instrumental resolve.
- V4 is finite 48 kHz stereo, all lyrics stay in their scenes, maximum sung-word pace is 105 WPM and FFT-bin energy above 12 kHz is `-76.05 dB`, passing the `-65 dB` ceiling. Two ACE-Step attempts remain rejected as all-NaN. Keep V4 local/unreleased until the user listens and approves it.
- A first master was rejected during manual review because pose dissolves ghosted/doubled the animals. V2 uses clean four-beat action cuts plus a brief golden wipe. The final general, every-boundary and action-transition sheets were reviewed with no ghost limbs. Full decode and all automated checks pass; producer and chant-song generator are reproducible under `automation/production/`.
- Keep it local and unreleased for the user's playback decision. `metadata/dance-with-dad-animal-parade-01.json` retains `upload_authorized: false` and `upload_queue_released: false`; do not queue or upload without new explicit approval.

## Moonlight Drum animal detective completed locally (2026-09-08)

- `moonlight-drum-animal-detectives-01` is a completed 100-second connected Australian-bush mystery using six animals not used by the earlier jungle/farm/bird clue-card trio: Kiri Quokka, Pippa Platypus, Edie Echidna, Billie Bilby, Suki Sugar Glider and Wally Wombat. The route is map nook → creek → fallen log → sandy burrow → canopy → clue junction → hollow-log reveal → parade. It is a continuous story, not four unrelated question cards.
- Producer: `automation/production/produce_moonlight_drum_detectives.py`; plan: `metadata/moonlight-drum-animal-detectives-01-plan.json`; accepted asset hashes: `metadata/moonlight-drum-animal-detectives-01-asset-review.json`; metadata: `metadata/moonlight-drum-animal-detectives-01.json`.
- Fifteen integrated full-frame images create explicit inspection/reveal, glide/landing, junction/departure and silent/drumming state changes. No blurred contextual backdrop or pasted foreground cutouts are used. General, every-boundary and three-state semantic contact sheets were manually inspected and passed.
- The revised final master is `automation/production-output/moonlight-drum-animal-detectives-01.mp4`, exactly 100.0 seconds, SHA-256 `d13dec70082011c711b02882271be3b6c69cf271e7ffe956e487a9298cff7dcd`. It passes full FFmpeg decode, H.264 1920x1080, 48 kHz stereo AAC, zero visual gaps, final-only four-second title hold and paired primary-action states.
- After user feedback, every spoken line now uses the same Maisie voice; there are no Ryan, Natasha or Ana voice changes. The revised delivery measures 115.69 weighted WPM, 116.16 maximum line WPM and 0.465-second minimum gaps. Audio is -16.3 LUFS / -0.8 dB true peak; a 9.8 kHz output low-pass plus steep isolated-band measurement places true opening energy above 12 kHz at `-66.41 dB`, below the `-65 dB` hiss ceiling.
- Thumbnail: `automation/thumbnails/moonlight-drum-animal-detectives-01.jpg`, hook `WHO PLAYS THE DRUM?`, SHA-256 `4694a16bcce382aa699d09dc00c042ee4a970ecb7f2b38a41e55af8531eebf79`. The master remains local/private: `upload_authorized: false` and `upload_queue_released: false`. Do not upload or publish without a new explicit user instruction after playback review.

## Animal Action Alphabet metadata relaunch (2026-09-08)

- The user explicitly authorized deleting and re-uploading `animal-action-alphabet-a-to-z-01` with a new thumbnail, title and description. The exact public Tiny Tales target was verified as `DTLx7vnHwd8`, with the title `Animal Action Alphabet A-Z | Move and Learn with 26 Animals`, made-for-kids status and a 5:31 duration.
- The archived master passed a complete FFmpeg decode at 1920x1080/30 fps with H.264 video and 48 kHz stereo AAC. A byte-identical, collision-safe replacement package was prepared as `animal-action-alphabet-a-to-z-01-relaunch`.
- After a non-destructive preflight verified the immutable Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`, exact old ID/title/status, ledger hash, replacement metadata and reviewed custom thumbnail, one delete request was sent for `DTLx7vnHwd8` and its absence was confirmed before upload.
- The replacement is public and made for kids at `dXuKOrtou_w` with the title `Move Like 26 Animals! | A–Z Animal Alphabet for Kids`. Independent API read-back confirmed the exact title, description, nine tags, category 27, 5:31 duration and succeeded processing. YouTube's served max-resolution thumbnail was fetched and visually matched the reviewed `26 ANIMAL MOVES!` artwork.
- The replacement source and sidecar are archived as `automation/archive/animal-action-alphabet-a-to-z-01-relaunch.mp4` and `.json`. The reviewed artwork and JPEG are `automation/thumbnail-artwork/animal-action-alphabet-a-to-z-01-relaunch.png` and `automation/thumbnails/animal-action-alphabet-a-to-z-01-relaunch.jpg`. The fail-closed one-purpose helper is `automation/replace_animal_action_alphabet.py`; its journal is under ignored `automation/runtime/animal-action-alphabet-relaunch.json`.
- Continue the user's next request one video at a time. Do not revive the retired static matching-card or silhouette-shadow formats. The next item should be a connected animal guessing adventure with a single story problem, physical clues, visible start/action/end states, one consistent lead voice and unhurried narration gaps.

## Golden Seed rainforest guessing adventure release (2026-09-08)

- The connected rainforest episode `golden-seed-rainforest-guessing-adventure-01` is complete and public at `https://youtu.be/xxdDW4ax1nI`. The exact 171.214-second reviewed master is archived at `automation/archive/golden-seed-rainforest-guessing-adventure-01.mp4`; SHA-256 is `f5464ed81d2cace33f7cf81593d17ff057557b47eef4d431eb9cfdbf6f8cb93c`.
- It follows one golden seed from Miki's opening catch through four physical obstacles. Cora Capybara, Arlo Armadillo, Sela Sloth and Kiko Kinkajou each have explicit start/action/end states that visibly move the same mission forward. The general, every-boundary, semantic-motion and exact encoded-master contact sheets were manually reviewed; no blank frames, pose ghosting or disconnected card structure were found.
- One Natasha (AU) voice is used throughout. Narration measures 112.34 weighted WPM, 112.58 maximum-line WPM and a 1.0-second minimum gap, with four exact six-second prediction holds. Final audio is -16.0 LUFS / -1.8 dB true peak, and opening energy above 12 kHz is -70.30 dB, passing the -65 dB hiss ceiling.
- The reviewed `WHO CAN HELP?` thumbnail SHA-256 is `948c057018e25d79ee9e2cdabcc1ff05f50e93d39b34599e9411adf7b2ea8297`. Independent API read-back confirmed the exact title, description, tags, category 27, public visibility, both made-for-kids flags, HD processing and PT2M52S duration on locked Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`. YouTube's served 1280x720 thumbnail was fetched and visually verified at RMS difference 3.163.
- The user authorized continuing with a few new sea-animal videos and uploading each once ready. Continue one at a time: build and review one connected sea-animal episode, release only its exact approved hash, verify the live copy, then proceed to the next. Avoid the already covered shadow, static matching-card, ocean superpower, counting-parade, bubble-size, habitat-rescue, lost-colour, disappearance-memory and generic movement formats.

## First new sea-animal episode in production (2026-09-08)

- Work started one-at-a-time on `luma-night-reef-lantern-rescue-01`, titled `Can Luma Light the Night Reef? | Sea Animal Adventure for Kids`. The authoritative connected-story plan is `metadata/luma-night-reef-lantern-rescue-01-plan.json`; upload is authorized once ready, but queue release remains false until the encoded master and custom thumbnail pass all reviews.
- Luma the leafy seadragon follows one continuous nursery-to-reef-to-nursery mission to recover exactly three unlit pearl lanterns. The distinct helpers are Gia Garden Eel, Deco Decorator Crab, Cato Cuttlefish and Fia Flashlight Fish. Use one Ryan (UK) voice at a 112 WPM target, four exact six-second prediction holds and a band-limited original 100 BPM score.
- Seventeen accepted full-frame assets are hash-locked in `metadata/luma-night-reef-lantern-rescue-01-asset-review.json`: cast reference, immediate opening problem, four complete start/action/end helper sets, corrected rehang action, glowing-lantern finale and thumbnail artwork. Continuity visibly progresses zero to one to two to three recovered lanterns. The first rehang attempt was rejected because it introduced a fourth lantern; preserve it under `automation/production-assets/rejected/luma-rehang-v1-four-lanterns.png` and never use it.
- Remaining work: implement the reproducible producer, render one continuous 1080p master, measure voice pacing/hiss/loudness, inspect general/transition/semantic/encoded-master sheets, render and inspect the final text thumbnail, then queue/upload/read back only if every gate passes. Do not start the second sea episode until this one is live-verified.

## Luma night-reef release and continuing authorization (2026-09-09)

- `luma-night-reef-lantern-rescue-01` is complete and public at `https://youtu.be/lHL3_9iQ26g`. The exact 170.733-second reviewed master is archived at `automation/archive/luma-night-reef-lantern-rescue-01.mp4`; SHA-256 is `435eafc99d6353833d6f1b84eda78a83d5c57f3958dff194ae56478fd073f1c4`.
- The master passed full decode, H.264 1920x1080/30 fps, 48 kHz stereo AAC, continuous zero-gap timing, final-only end card, four exact six-second prediction holds, four complete semantic start/action/end sets and exact zero-one-two-three lantern continuity. Manual review covered the source-level general/every-boundary/semantic sheets and a 12-frame contact sheet sampled from the actual encoded master; no blanks, pose ghosts, disconnected sequence or count error was found.
- One Ryan (UK) voice is used throughout at 112.34 weighted WPM, 112.54 maximum-line WPM and a 1.0-second minimum narration gap. Final audio is -16.2 LUFS / -1.5 dB true peak; opening energy above 12 kHz is -70.54 dB and passes the -65 dB hiss ceiling.
- Independent API read-back confirmed exact title, description, eight tags, category 27, public visibility, both made-for-kids flags, succeeded HD processing and PT2M51S duration on locked Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`. The custom thumbnail SHA-256 is `4271c101ba3c58628ca9ace947764327e15ee57b557b77ba16afb97b494649a9`; YouTube's served 1280x720 copy was fetched, visually matched and measured RMS difference 4.409.
- On 2026-09-09 the user reaffirmed that similar new videos should continue and upload as soon as each is done, with every quality check. This is standing authorization for the next structurally distinct animal-adventure episodes one at a time. Do not interpret it as batch generation or bypass any review gate; finish, upload and live-verify each exact master before moving to the next.

## Under-ten-view metadata refresh (2026-09-09)

- The user authorized improving the titles, description leads and tags of the 17 unique public upload-ledger videos found below 10 views. The exact fail-closed plan is `metadata/under-10-metadata-refresh-2026-09-09.json`; the live audit inputs remain under ignored `automation/runtime/under-10-view-*.json`.
- All 17 exact titles and description leads were updated on locked Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`: `lHL3_9iQ26g`, `xxdDW4ax1nI`, `h2RfEK01Rr0`, `ggJqahpLAtg`, `2CKnDBpjFlQ`, `aPtUBaZrPcI`, `9vvghOKhipo`, `IljnXnB_5X0`, `RjHEcm4aWUs`, `nmj4W890T3I`, `FdgXOzSIpy0`, `Ow7tr3SWiM4`, `eCGiwPW_SF4`, `KT6ezPwGYZI`, `3c5rj6In7pQ`, `yUCTNP3pJxg` and `ygc-y4_XBwk`.
- Existing relevant tags were deliberately preserved because they already matched each video's actual content; category 27, public visibility and made-for-kids status were also preserved. Thumbnails were not changed in this pass. A fresh independent API read-back matched all 17 complete canonical titles and descriptions, tags, category, visibility, audience setting and channel identity. The result is `automation/runtime/under-10-metadata-refresh-result.json`.
- The immediate batch read-back correctly exposed that Nia's old live description had no second paragraph to preserve. Its short canonical Tiny Tales paragraph was added with one exact-ID repair, then all 17 full descriptions passed exact readback. `automation/refresh_under_10_metadata.py` now retries read-only verification briefly before declaring a possible propagation mismatch.

## Seventeen-video hourly relaunch (2026-09-09)

- The user explicitly instructed that the same 17 under-ten-view uploads be deleted and uploaded again with the refreshed titles and descriptions at one-hour intervals. The exact plan is `metadata/under-10-hourly-relaunch-2026-09-09.json`; the fail-closed, resumable helper is `automation/relaunch_under_10_hourly.py`; the mutation journal and final audit are in ignored `automation/runtime/under-10-hourly-relaunch-result.json`.
- Before deletion, every exact old ID was verified public, made for kids, on locked Tiny Tales channel `UCEn9N-ITQHshjgt6fy7fxnw`, below ten views, and matched its complete refreshed metadata. All 17 archived source hashes and reviewed 1280x720 JPEGs matched, FFprobe passed, and fresh full FFmpeg decodes passed.
- Each old upload was deleted exactly once and confirmed absent before its byte-identical reviewed master was re-uploaded. A later old upload was never deleted until the prior replacement had returned a new ID, received its custom thumbnail and passed live readback. Two temporary tag-readback delays were reconciled against the existing returned IDs; neither upload nor delete was repeated.
- Exact old-to-new mapping: `lHL3_9iQ26g` -> `rPqCqUGhywE`; `xxdDW4ax1nI` -> `DMFBnL1pIq4`; `h2RfEK01Rr0` -> `u36XhMCos_M`; `ggJqahpLAtg` -> `zzd0QNMHLPo`; `2CKnDBpjFlQ` -> `OB3pJXp5A3c`; `aPtUBaZrPcI` -> `wfKsQhCaYp0`; `9vvghOKhipo` -> `em2xnDnCUCI`; `IljnXnB_5X0` -> `iVBkoJPFRek`; `RjHEcm4aWUs` -> `IIDM9d8L6dE`; `nmj4W890T3I` -> `NwKCjPWkbvU`; `FdgXOzSIpy0` -> `xeXRw2oM_6Y`; `Ow7tr3SWiM4` -> `AQ7VTUeudnU`; `eCGiwPW_SF4` -> `Crs3i1GRCF4`; `KT6ezPwGYZI` -> `wM6egGPxJYQ`; `3c5rj6In7pQ` -> `ilb9JgVSuLk`; `yUCTNP3pJxg` -> `H5cc7waTsCQ`; `ygc-y4_XBwk` -> `qWRAn9r1GlU`.
- The replacements use YouTube's native private-to-public scheduling, not a Windows Scheduled Task. They publish hourly from `2026-09-09T08:00:00Z` (18:00 Sydney) through `2026-09-10T00:00:00Z` (10:00 Sydney). Final independent API readback found all 17 old IDs absent and all 17 replacements present, processed, private until schedule, made for kids, with exact titles, descriptions, tag sets, category 27 and publish times.
- All 17 thumbnail API confirmations were recorded. The served max-resolution copies were fetched at 1280x720 and compared with the reviewed local JPEGs; all passed, with RMS differences 2.397-5.206 from normal YouTube recompression. Do not re-upload these hashes again or recreate any Tiny Tales Scheduled Task on this computer.
