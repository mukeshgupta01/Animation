...
## Fruit Picnic active visual checkpoint (2026-08-28)

- `pick-wash-pack-fruit-picnic-01` has a locked 97.333-second / 90 BPM orchard action-song plan: seven exact five-bar / 13.333-second scenes plus a four-second resolve. Maisie leads, Ryan voices Kai and Ana voices Rosa. Preserve its layered luminous watercolour-and-gouache storybook direction and the exact plan in `metadata/pick-wash-pack-fruit-picnic-01-plan.json`.
- Three 1672x941 scenes are accepted: permission at the gate, one supported-branch apple twist, and an exact count tableau. The accepted count contains only two red apples, two golden pears and two purple plums on the table, exactly three pair tokens and an empty basket. The first generated count variant was rejected for extra basket fruit and was never copied into the project.
- Continue with wash, red-gold-purple pack pattern, shared carry and picnic finale. Exact accepted hashes and continuity requirements are in `metadata/pick-wash-pack-fruit-picnic-01-asset-review.json`. Nothing is queued or uploaded.
