...
- `metadata/ant-grasshopper-pika-pilot-02.json` is the locked simplified follow-up: only Ant attempts one slow planted weighted step while the other four characters remain supportively still. Do not generate the remaining story assets, render the full story, queue footage, or upload Pika output unless pilot 02 visibly passes locomotion, anatomy, prop, camera and final-pose checks.
