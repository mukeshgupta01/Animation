...
$dailySummaryScript = Join-Path $PSScriptRoot 'Send-DailyUploadSummary.ps1'
...
$uploadStart = $uploadAnchor.Date.AddHours($uploadAnchor.Hour).AddMinutes([math]::Ceiling($uploadAnchor.Minute / 15.0) * 15)
$uploadTriggers = @(
...
$dailySummarySettings = New-ScheduledTaskSettingsSet -StartWhenAvailable -WakeToRun -RunOnlyIfNetworkAvailable -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -MultipleInstances IgnoreNew -ExecutionTimeLimit (New-TimeSpan -Minutes 15)
