...
REVIEWED_MASTER_SHA256 = "d2c70155de568232ef2e16ae5b76d2847875e51f59f7ae4289c90879877ea5b4"
REVIEWED_THUMBNAIL_SHA256 = "94c75de299e89b89be094a32250e4c07fa46a23b9f3c7e4ce9c9c27fa9a33fe7"
BPM = 90
BEAT = 60 / BPM
EIGHTH = BEAT / 2
SCENE_SECONDS = 12.0
...
def quality(events,total,assets):
    ...
    reviewed=(hashlib.sha256(OUTPUT.read_bytes()).hexdigest()==REVIEWED_MASTER_SHA256 and
              hashlib.sha256(THUMBNAIL.read_bytes()).hexdigest()==REVIEWED_THUMBNAIL_SHA256)
    audit=[]
    plan=load_plan()
    for event,scene in zip(events[:-1],plan["scenes"]):
        audit.append({"scene":scene["scene"],"primary_action":scene["action"],"visible_start_state":"integrated full-frame story setup before the narrated action","visible_action_state":"scene-specific light, pattern, shelter or path object visibly changes on the narration beat","visible_end_state":"the physical story state settles before the next scene","foreground_moving_elements":["story lanterns","pattern leaves","light path"],"camera_only":False,"character_and_object_continuity":True,"reviewed":reviewed})
...
def write_metadata(total,report):
    ...
    doc={"id":ITEM_ID,"title":"Felix Firefly and the Three Dark Lanterns | Story for Kids","description":"Felix finds Rabbit and Hedgehog beside three dark leaf lanterns. Together they remember two colour patterns, help Felix rest, and relight the safe woodland path home.\n\nAn original Tiny Tales story about patterns, courage, friendship and helping one another for children ages 3 to 7.","tags":["firefly story for kids","pattern story","bedtime story for kids","friendship story","woodland animals","colours for kids","Tiny Tales"],"category_id":"27","made_for_kids":True,"privacy":"public","upload_authorized":reviewed,"output":str(OUTPUT),"duration_seconds":total,"voice_profile":"ana-us","format_family":"story-led woodland pattern adventure","quality_gate_passed":True,"full_decode_passed":True,"transition_audit_passed":True,"quality_report":f"automation/production-work/{ITEM_ID}/quality-report.json","transition_audit":f"automation/production-work/{ITEM_ID}/timeline-gap-audit.json","narration_visual_sync_audit":f"automation/production-work/{ITEM_ID}/lyric-visual-emotion-audit.json","narration_pacing_audit":f"automation/production-work/{ITEM_ID}/narration-pacing-audit.json","quality_contact_sheet":f"automation/production-work/{ITEM_ID}/quality-contact-sheet.png","transition_contact_sheet":f"automation/production-work/{ITEM_ID}/transition-contact-sheet.png","semantic_motion_audit":f"automation/production-work/{ITEM_ID}/semantic-motion-audit.json","semantic_motion_contact_sheet":f"automation/production-work/{ITEM_ID}/semantic-motion-contact-sheet.png","prepared_thumbnail":f"automation/thumbnails/{ITEM_ID}.jpg","thumbnail_hook":"THREE DARK LANTERNS","thumbnail_reviewed":reviewed,"transition_contact_sheet_reviewed":reviewed,"quality_contact_sheet_reviewed":reviewed,"semantic_motion_reviewed":reviewed,"character_continuity_reviewed":reviewed,"primary_action_motion_reviewed":reviewed,"actual_motion_not_camera_only":reviewed,"manual_visual_review_passed":reviewed,"reviewed_sha256":master_hash,"reviewed_thumbnail_sha256":thumbnail_hash,"manual_review_notes":"The user explicitly accepted this exact one-at-a-time Felix master and thumbnail for upload on 2026-09-02 after reviewing the rebuilt story-led version.","integrated_loudness_lufs":report["integrated_loudness_lufs"],"true_peak_dbfs":report["true_peak_dbfs"],"opening_high_band_rms_db_above_12khz":report["opening_high_band_rms_db_above_12khz"],"new_image_generation_calls":1,"true_rigged_3d_animation":False,"paid_generation_used":False,"upload_queue_released":reviewed}
