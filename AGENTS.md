...
- THIS-COMPUTER TINY TALES STOP (2026-08-31): do not install, enable, run or recreate Tiny Tales Scheduled Tasks on this computer. LOCAL REMOVAL (2026-09-23, C:/DocSphere computer only): the user requested removal of all local Tiny Tales files, including finished media and artwork. The KidsRhymes working folder is removed and excluded through local Git sparse checkout. Preserve the shared remote project/history and the online channel; do not restore the local folder without a new request. Keep tools shared with Birthday Songs and Parenting Rewind. Schedule only the separately authorized Parenting Rewind and repository-sync automation here.
